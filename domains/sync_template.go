...
type SyncTemplate struct {
	...
	Name                       string `json:"name" gorm:"size:128;index;comment:模板名称"`
	Description                string `json:"description" gorm:"size:512;comment:模板说明"`
	...
	CursorValue                string `json:"cursorValue" gorm:"size:255;comment:默认游标值"`
	...
	SyncTimeField              string `json:"syncTimeField" gorm:"size:128;comment:TDengine同步时间字段"`
	SyncStartDate              string `json:"syncStartDate" gorm:"size:32;comment:TDengine同步开始日期"`
	SyncEndDate                string `json:"syncEndDate" gorm:"size:32;comment:TDengine同步结束日期"`
	TDengineChildTableTemplate string `json:"tdengineChildTableTemplate" gorm:"size:255;comment:TDengine目标子表名模板"`
	TDengineChildTableField    string `json:"tdengineChildTableField" gorm:"size:128;comment:TDengine目标子表名源字段"`
	TDengineTags               string `json:"tdengineTags" gorm:"type:text;comment:TDengine目标TAG映射JSON"`
...
func (SyncTemplate) TableName() string { return "data_sync_templates" }

func (s SyncTemplate) GetBaseData() commonDomains.BaseDataEntity {
